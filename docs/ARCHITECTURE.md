...
**Observability splits by altitude.** The dashboard (`hflow ui`, see [UI.md](./UI.md)) covers everything down to the task, and answers each altitude in its own terms. At stage altitude the question is where the data is and what is holding it up, so a run reads as its stages: what each one is for, which verification family vouches for it, how many episodes are through, and the pace, estimate, and stall warning that locate a bottleneck. Those counts are a read of the catalog the pipeline already writes -- one append per episode a stage finishes -- so the view costs no new instrumentation and the dashboard stays a viewer with no state of its own. A stage card is also deliberately free of Airflow vocabulary: a verification stage that lives behind an external service (human review, say) fills the same shape from its own API. At task altitude the question is what the orchestrator did, so the same run also draws its DAG with per-task state, from Airflow's own structure and task-instance APIs -- the blog cites "the status of each step is clearly observed from the DAG" as a benefit, and this is that view without the detour. Below the task, Airflow's own UI remains the tool -- logs, retries, re-runs, exposed on localhost in Compose mode and linked from every task. The SDK also adds plain-language diagnostics for embedder-specific traps (`hflow status`).
