...
- **A local dashboard for operating it all.** `hflow ui` shows live runs with per-stage status, storage roots with catalog stats, and per-user secrets for task containers -- a thin localhost viewer over the Airflow REST API and the Parquet catalog ([docs](./docs/UI.md)).
