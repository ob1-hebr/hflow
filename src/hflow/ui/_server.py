...
_ROUTES: list[tuple[str, re.Pattern[str], Any]] = [
    ...
    ("GET", re.compile(r"^/api/pipelines/runs/(?P<run_id>[^/]+)/graph$"), run_graph_handler),
    (
        "GET",
        re.compile(r"^/api/pipelines/runs/(?P<run_id>[^/]+)/stages/(?P<stage>[a-z]+)/graph$"),
        stage_graph_handler,
    ),
    (
        "GET",
        re.compile(r"^/api/pipelines/runs/(?P<run_id>[^/]+)/stages/(?P<stage>[a-z]+)/checks$"),
        stage_checks_handler,
    ),
